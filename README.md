...
## Dependencies:
    To install the needed dependecies use the command: ```pip install -r requirements.txt```
    If any error will occure try on the Linux system: ```sudo apt-get install libsndfile1 ffmpeg``` or for a macOS: ```brew install libsndfile ffmpeg```
